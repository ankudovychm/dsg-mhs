...
// Clear existing list items in the dropdown (optional, if you want to refresh the UI)
list.innerHTML = ''; // Clear existing list items if needed

// Now process options and set their states
Array.from(el.options).forEach(o => {
  ...
  if (window.Extent.includes(parseInt(o.value))) {
      ...
      // Keep checkbox state consistent
      ic.addEventListener('click', (ev) => {
          ic.checked = !ic.checked;
      });

      // If the option is visible, mark it as selected
      if (!op.classList.contains('hidden')) { // Check for visibility
          ic.checked = true; // Mark as selected if visible
          op.optEl.selected = true; // Set the original option to selected
      }
